    ...
    def run_check(self, result):
        ...
        if data_instance_delta != 0:
            ...
            if remaining_seconds > 0:
                result.ok('%s: %s, but still %s remaining to fulfill requirements' % (base_text, text, str(remaining)))
                return
            result.critical('%s: %s' % (base_text, text))
            return
        result.ok('%s: %d data instances, just as expected' % (base_text, data_instance_count))
